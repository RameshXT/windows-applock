...
pub fn verify_impl(
    ...
    println!(
        "[Auth] Attempting verification for password length: {}",
        password.len()
    );
    ...
    if let Some(until) = config.lockout_until {
        ...
        if now < until {
            ...
            println!(
                "[Auth] Denied: Security lockout active for {} more seconds",
                remaining
            );
            return Err(format!(
                "Security lockout active. Try again in {} seconds.",
                remaining
            ));
        } else {
    ...
    if is_valid {
        ...
        *unlocked = true;
        ...
        println!(
            "[Auth] Credentials MISMATCH. Attempts: {}/{}",
            attempts, limit
        );
