...
interface UseAppsResult {
  ...
  setAppToRemove: React.Dispatch<React.SetStateAction<LockedApp | InstalledApp | null>>;
...
/**
 * Owns all app-list state. Pass `triggerToast` and `setError` once at creation.
 * All handlers operate on internal state — no need to pass lockedApps as params.
 */
export function useApps(
  ...
  const toggleApp = async (app: LockedApp | InstalledApp, fromTab?: Tab) => {
    // Use functional setter to always read latest state
    let snapshot: LockedApp[] = [];
    setLockedApps(prev => { snapshot = prev; return prev; });

    const isLocked = snapshot.some(la => la.name === app.name);

    if (isLocked) {
      if (fromTab === "all") { setAppToRemove(app); return; }
    ...
    const newLocked: LockedApp[] = [...snapshot, {
